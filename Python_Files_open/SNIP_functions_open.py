...
# The model was modified for Python 3.11 to run on open source packages.
...
def addDEMPntstoNodes(nodes, archPathListMST, boundingCandidates, fromNode, toNode, minTD):
    """
    Add points on path to nodes. In case the node is a DEM-Points, get DEM Coordinates. .
    ...
    nodes                 --    Updated nodes
    """
    if len(archPathListMST) > 0:
        for edge in archPathListMST:
            # Add both vertices for the edge
            for vertex_id, height in [(edge[0], edge[2]), (edge[1][0], edge[3])]:
                if all(n[0] != vertex_id for n in nodes):
                    # Try DEM coordinates first
                    added = False
                    for cell in boundingCandidates:
                        if vertex_id != fromNode and vertex_id != toNode and cell[0] == vertex_id:
                            nodes.append([
                                vertex_id, cell[1], cell[2], height,
                                0, 0, 0, 0, 0, [], height - minTD
                            ])
                            added = True
                            break
                    # If no DEM found, add vertex with None coords
                    if not added:
                        nodes.append([
                            vertex_id, None, None, height,
                            0, 0, 0, 0, 0, [], height - minTD
                        ])
    return nodes
...
def SNIP(OnlyExecuteMerge, outListFolder, runNr, nodes, anteilDaten, streetNetwork, startnode, edgeList, streetVertices,
    ...
    f_merge = inParameter[
        3]  # Factor to determine how the OST are reconnected. Puts the size in relation ot the distance. If large, the size gets more important
    ...
    pumpYears = inParameter[
        8]  # Number of years the pump needs to pump [years]. Needed in order to compare lifespan of pipes to pumping costs
    ...
    print("fc_SewerCost: " + str(f_SewerCost))
    print("fc_wwtpOpex: " + str(fc_wwtpOperation))
    print("fc_wwtpCapex: " + str(fc_wwtpReplacement))
    print(" ")
    print(" ")
...
def createStreetGraph(in_FC, crs):
    """
    Reads out coordinates from line shapefile fields.
...
def createPolyPoint(pntsFlow, outListStep_point, crs):
    """
    Create point shapefile from list of coordinates.
    ...
    for entry in pntsFlow:
        pointList.append(Point(entry[1], entry[2]))

    if len(pointList) == 0:
        draw = 0
        return draw

    gdf = gpd.GeoDataFrame(geometry=pointList)
    gdf = gdf.set_crs(crs=crs)
    gdf.to_file(outListStep_point)
    return draw


def createPolyPointWWTP(pntsFlow, outListStep_point, crs):
    """
    Create point shapefile from list of coordinates.
    ...
    for entry in pntsFlow:
        pointList.append(Point(entry[2], entry[3]))
...
def createPolyPointPump(pntsFlow, outListStep_point, crs):
    """
    Create point shapefile from list of coordinates.
...
def writeWWTPs(shapefile_path, pointListNear, crs):
    """
    ...
    n = min(len(gdf), len(pointListNear))
    gdf.loc[:n - 1, "ID"] = [pt[0] for pt in pointListNear[:n]]
    gdf.loc[:n - 1, "FLOW"] = [pt[1] for pt in pointListNear[:n]]
    gdf = gdf.set_crs(crs, allow_override=True)
    gdf.to_file(shapefile_path)
    return


def writeStartnode(shapefile_path, startnodeID, crs):
    """
    Create point shapefile.
...
def readOutAllStreetVerticesAfterAggregation(nodes, rasterPoints, rasterSize, crs):
    """
    ...
    for _, row in gdf.iterrows():
        ...
        if copyZ2 == 1:
            IDNEW += 1
            heightEND = find_height(X_end, Y_end, rasterPoints, rasterSize)
...
def mergePathClosestNetwork(pathInClosestNetwork, archPathWWTP, edgeList):
    """
    This function appends to the path from a WWTP to the node of a closest network the path in the closest network to the
...
def readStreetPoints(nodes, crs):
    """
...
def addedgesID(edges, streetVertices):
    """
    This function assigns Ids and distance to edges
...
def standardize_id_field(gdf):
    """
    Standardize ID field to 'FID' regardless of whether input has FID, ORIG_FID, or OBJECTID.
    """
    if "ORIG_FID" in gdf.columns:
        gdf = gdf.rename(columns={"ORIG_FID": "FID"})
    elif "OBJECTID" in gdf.columns:
        gdf = gdf.rename(columns={"OBJECTID": "FID"})
    # If FID already exists, leave it
    return gdf


def readBuildingPoints(in_FC, crs):
    """
    Reads building points from a shapefile.
    ...
    for _, row in gdf.iterrows():
        ...
        X = row["POINT_X"]
        Y = row["POINT_Y"]
        quantity = row.get("Q", 0)  # flow amount, default to 0 if missing
        pointListNear.append((ID, X, Y, 0, quantity))  # ID, x, y, pop_orig=0, Q

    return pointListNear


def readClosestPointsAggregate(in_FC, crs):
    """
    ...
    for _, row in gdf.iterrows():
        ID = row["FID"]
        X = row.get("POINT_X", None)
        Y = row.get("POINT_Y", None)
        Q = row.get("Q", 0)
        NEAR_X = row.get("NEAR_X", -1)
        NEAR_Y = row.get("NEAR_Y", -1)

        # Check if Near was not calculated
        if NEAR_X == -1 or NEAR_Y == 1:  # matches original logic
...
def aggregate(pointListNear, AggregateKritStreet, outListStep_point, minTD, crs):
    """
    ...
    """
    pointList, buildingList, aggregatetStreetInlets = [], [], []

    for i in pointListNear:
        IdGeb, p0, StartDist = i[0], (i[1], i[2], i[3]), float('inf')

        closestX = closestY = closest_height = closest_quan = None

        if len(aggregatetStreetInlets) > 0:
            for e in aggregatetStreetInlets:
                p1 = (e[1], e[2], e[3])
                distanz = distanceCalc2d(p0, p1)  # Using pre-defined function
            ...
            if StartDist > AggregateKritStreet:
                ...
                # Merge with closest street inlet
                for e in aggregatetStreetInlets:
                    if e[1] == closestX and e[2] == closestY:
                        ...
                        # Update inlet building IDs
                        e[9].append(IdGeb)
                        # Sum quantities
                        e[8] = i[8] + closest_quan
                        # Sum heights
                        e[3] += i[3]
                        break
        else:
    ...
    # Assign unique IDs
    for idx, i in enumerate(aggregatetStreetInlets):
        i[0] = idx

    # Divide summed height by number of buildings
    for i in aggregatetStreetInlets:
        summe, anzBuildinging = i[3], len(i[9])
        i[3] = float(summe) / float(anzBuildinging)

    # Create GeoDataFrame and save points
    points = [Point(entry[1], entry[2]) for entry in aggregatetStreetInlets]
...
def drawAllNodes(nodes, allNodesPath, crs):
    """
    ...
    allNodesPath       --    path
    """
    points = [Point(entry[1], entry[2]) for entry in nodes]
    gdf = gpd.GeoDataFrame(geometry=points)
    gdf = gdf.set_crs(crs=crs)
    gdf.to_file(allNodesPath)


def splitStreetwithInlets(in_FC, outListStep_point, aggregatetStreetFile, crs):
    """
    Splits a line shapefile at given points (street inlets).
    ...
    """
    # Read input
    streets_gdf = gpd.read_file(in_FC).set_crs(crs)
    points_gdf = gpd.read_file(outListStep_point).set_crs(crs)

    # Snap points within 2 m to nearest line
    snapped_points = []
    for pt in points_gdf.geometry:
        nearest_line_idx = streets_gdf.distance(pt).idxmin()
        if streets_gdf.geometry[nearest_line_idx].distance(pt) <= 2:  # 2 m search radius
            projected_pt = streets_gdf.geometry[nearest_line_idx].interpolate(
                streets_gdf.geometry[nearest_line_idx].project(pt)
            )
            snapped_points.append(projected_pt)
    points_gdf = gpd.GeoDataFrame(geometry=snapped_points, crs=crs)

    # Split lines at snapped points
    split_lines = []
    for line in streets_gdf.geometry:
        # Filter points on this line (tolerance)
        pts_on_line = [pt for pt in points_gdf.geometry if line.distance(pt) < 1e-6]
        if pts_on_line:
            multipoint = MultiPoint(pts_on_line)
            result = split(line, multipoint)
            # Extend lines
            split_lines.extend([seg for seg in result.geoms])
        else:
            split_lines.append(line)

    # Save split streets
    split_streets_gdf = gpd.GeoDataFrame(geometry=split_lines, crs=crs)
    split_streets_gdf.to_file(aggregatetStreetFile)

    return split_streets_gdf


def updatefieldsPoints(shapefile_path, crs):
    """
    ...
    # Add required fields (initialize with None or 0)
    gdf["ID"] = None
    gdf["StreetID"] = None
    gdf["NEAR_DIST"] = None
    gdf["LENGTH"] = gdf.geometry.length
    gdf["X_START"] = gdf.geometry.apply(lambda geom: geom.coords[0][0] if geom else None)
    gdf["Y_START"] = gdf.geometry.apply(lambda geom: geom.coords[0][1] if geom else None)
    gdf["X_END"] = gdf.geometry.apply(lambda geom: geom.coords[-1][0] if geom else None)
    gdf["Y_END"] = gdf.geometry.apply(lambda geom: geom.coords[-1][1] if geom else None)

    # Save back to the same path (overwrite)
    gdf.to_file(shapefile_path)


def updateFieldNode(shapefile_path, crs):
    """
    This function creates fields in a shapefile.
    ...
    # Add required fields
    gdf["ID"] = None  # Equivalent to creating "ID" field

    # ArcPy deletes "Id" automatically created field; remove if exists
...
def updateFieldStreetInlets(shapefile_path, crs):
    """
    This function creates fields in a shapefile.
    ...
    """
    gdf = gpd.read_file(shapefile_path)
    gdf = gdf.set_crs(crs=crs)  # set CRS on GeoDataFrame

    # Add required fields
    gdf["SourceFlow"] = None
    gdf["ID_AGGREG"] = None  # Equivalent to creating "ID_AGGREG" field

    # ArcPy deletes automatically created "Id" field; remove if exists
    if "Id" in gdf.columns:
        gdf = gdf.drop(columns=["Id"])
...
def writefieldsAllNodes(shapefile_path, pointListNear, crs):
    """
    ...
    # Assign values from pointListNear to "ID" field
    for idx, val in enumerate(pointListNear):
        if idx < len(gdf):
            gdf.at[idx, "ID"] = val[0]
...
def writefieldsStreetInlets(shapefile_path, pointListNear, crs):
    """
    ...
    if "SourceFlow" not in gdf.columns:
        gdf["SourceFlow"] = None
    if "ID_AGGREG" not in gdf.columns:
        gdf["ID_AGGREG"] = None

    # Assign values from pointListNear
    for idx, val in enumerate(pointListNear):
        if idx < len(gdf):
            gdf.at[idx, "SourceFlow"] = val[8]
            gdf.at[idx, "ID_AGGREG"] = val[0]

    # Save back to same path (overwrite)
    gdf.to_file(shapefile_path)


def writefieldsNodes(shapefile_path, pointListNear, crs):
    """
    This function creates fields in a shapefile and adds a value from a list
    ...
    for field in ["ID", "POP", "FLOW", "TRENCHD", "HEIGHT"]:
    ...
    for idx, val in enumerate(pointListNear):
        if idx < len(gdf):
            gdf.at[idx, "ID"] = val[0]
            gdf.at[idx, "HEIGHT"] = val[3]
            gdf.at[idx, "FLOW"] = val[4]
            gdf.at[idx, "POP"] = val[6]
            gdf.at[idx, "TRENCHD"] = val[7]

    # Save back to same path (overwrite)
    gdf.to_file(shapefile_path)


def writeFieldNodesPUMPS(shapefile_path, pointListNear, crs):
    """
    This function creates fields in a shapefile and adds a value from a list
...
def createPolyLine(polyline_calc, outListStep, crs):
    """
    ...
    geometries = []
    FLOW, DIAMETER, TRENCH = [], [], []

    for feature in polyline_calc:
        # feature[1] and feature[2] are coordinate pairs
        line = LineString([feature[1], feature[2]])
        geometries.append(line)
        FLOW.append(feature[3])
        DIAMETER.append(feature[4])
        TRENCH.append(feature[5])
...
def readRasterPoints(in_FC, anzForID, crs):
    """
    ...
    # Calculate rasterCellSize using first two points
    if len(pointListNear) < 2:
        raise Exception("Error: Not enough raster points to calculate cell size.")

    firstX, firstY = pointListNear[0][1], pointListNear[0][2]
    secondX, secondY = pointListNear[1][1], pointListNear[1][2]
    rasterCellSize = abs(secondX - firstX)
    if rasterCellSize == 0:
        rasterCellSize = abs(secondY - firstY)
...
def getStatistics(startnode, sewers, pointsPrim, aggregatetPoints, WWTPs, edgeList, nrOfNeighboursDensity, EWQuantity,
    ...
    '''
    print("\nStatistics\n----------")
    totalPublicPipeLength, totalPrivatePipeLength = 0.0, 0.0

    # Sum public pipe lengths
    for i in sewers:
        if sewers[i] != ((), 0):
            totalPublicPipeLength += sewers[i][1]

    # Sum private pipe lengths (house connections)
    for node in buildings:
        pt_to1_X, pt_to1_Y, gebListe = node[0], node[1], node[2]
        ...
        for house in gebListe:
            for geb in buildPoints:
                if geb[0] == house:
                    ...
                    totalPrivatePipeLength += distance
                    break

    print(f"\nTotal public pipe Length:  {totalPublicPipeLength}")
    print(f"Total private pipe Length: {totalPrivatePipeLength}\n")

    # Degree of centralization
    sources = len(aggregatetPoints)
    sinks = len(WWTPs)
    degCen = round((float(sources) - float(sinks)) / float(sources), 4)
    print(f"Non-weighted Degree of centrality: {degCen}")

    # Weighted degree of centrality
    listWWTPwithAggregatedNodes = getAggregatedNodesinListWWTP(WWTPs, sewers, aggregatetPoints)
    sumFlow, weightedTerm = 0, 0
    for i in listWWTPwithAggregatedNodes:
        sumFlow += i[1]
        weightedTerm += float(i[1] / i[2])
    degCenWeighted = (sumFlow - weightedTerm) / sumFlow
    print(f"Weighted degree of centrality: {degCenWeighted}\n")

    # Average trench depth
    cnt, averageTrenchDepth = 0.0, 0.0
    for i in pointsPrim:
        if i[7] != 0:
            averageTrenchDepth += i[7]
            cnt += 1
    averageTrenchDepth /= cnt
    print(f"Average Trench Depth: {averageTrenchDepth}")

    # Average height of sewer network
    cnt, averageHeight = 0.0, 0.0
    for i in pointsPrim:
        if i[7] != 0:
            averageHeight += i[3]
            cnt += 1
    averageHeight /= cnt
    print(f"Average height of sewer network: {averageHeight}")

    # Average WWTP size
    cnt, averageWWTPSize, totFlow = 0.0, 0.0, 0.0
    for i in WWTPs:
        averageWWTPSize += i[1]
        totFlow += i[1]
        cnt += 1
    ...
    statistics = [
        ...
        "sinks", sinks,
        "degCen", degCen,
        "degCenWeighted", degCenWeighted,
        "nrOfNeighboursDensity", nrOfNeighboursDensity,
        "totalPublicPipeLength", totalPublicPipeLength,
        "totalPrivatePipeLength", totalPrivatePipeLength,
        "averageTrenchDepth", averageTrenchDepth,
